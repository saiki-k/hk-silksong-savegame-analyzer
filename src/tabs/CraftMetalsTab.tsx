...
const CATEGORY_NAME = "Craftmetals";

export function CraftMetalsTab({ parsedJson, decrypted }: TabRenderProps) {
  if (!decrypted || !parsedJson) {
    return <div className="text-white text-center">Load a save file to view Relic data.</div>;
  }

  const craftMetalsCategory = CATEGORIES.find(cat => cat.name === CATEGORY_NAME);
  const craftmetals = craftMetalsCategory?.items ?? [];
...
export function getCraftMetalsExtra({ parsedJson, decrypted }: { parsedJson: unknown; decrypted: boolean }): ReactNode {
  if (!decrypted || !parsedJson) {
    return null;
  }
  ...
  if (craftmetals.length === 0) {
    return null;
  }

  let unlockedCount = 0;

  craftmetals.forEach(item => {
    const { unlocked } = isItemUnlockedInPlayerSave(item.parsingInfo, parsedJson);
    if (unlocked) {
      unlockedCount += 1;
    }
  ...
  return (
    <div className="text-xs text-blue-200 mt-1 font-normal">
      {`${unlockedCount}/${craftmetals.length}`}
    </div>
  );
}
