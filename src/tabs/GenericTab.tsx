...
import type { NormalisedTrackableCategory, CategoryItem } from "../parsers/types";
...
// Helper function to normalize TrackableCategory to NormalisedTrackableCategory
function normalizeCategory(category: any): NormalisedTrackableCategory {
  if (category.sections) {
    // Already normalized
    return category as NormalisedTrackableCategory;
  }
...
export function GenericTab({ parsedJson, decrypted, tabLabel }: GenericTabProps) {
  ...
  const categoryData = CATEGORIES.find(cat => cat.name === tabLabel);
  if (!categoryData) {
...
export function getGenericExtra({ 
  parsedJson, 
  decrypted, 
  tabLabel,
  useCount = false
  ...
  const allItems = normalizedCategory.sections.flatMap(section => section.items);
