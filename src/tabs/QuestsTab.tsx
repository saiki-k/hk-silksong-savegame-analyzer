...
const CATEGORY_NAME = "Quests";

type QuestItem = (typeof CATEGORIES)[number]["items"][number];

function QuestsTableSection({ section, quests, parsedJson }: { section: string; quests: QuestItem[]; parsedJson: any }) {
...
export function QuestsTab({ parsedJson, decrypted }: TabRenderProps) {
  if (!decrypted || !parsedJson) {
    return <div className="text-white text-center">Load a save file to view Quests data.</div>;
  }

  const questCategory = CATEGORIES.find(cat => cat.name === CATEGORY_NAME);
  const quests = questCategory?.items ?? [];

  const sections = Array.from(new Set(quests.map(w => w.section).filter((s): s is string => typeof s === "string")));
    ...
    <div className="text-white">
      {sections.map(section => (
        <QuestsTableSection
          key={section}
          section={section}
          quests={quests.filter(w => w.section === section)}
...
export function getQuestsExtra({ parsedJson, decrypted }: { parsedJson: unknown; decrypted: boolean }): ReactNode {
  if (!decrypted || !parsedJson) {
    return null;
  }
